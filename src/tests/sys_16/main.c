...
int test_hashmap_basic_operations(void) {
  ...
  sys_hashtable_t *map = sys_hashmap_init(8);
  test_assert(map != NULL);

  // Test basic put and get
  uintptr_t replaced = 0;
  bool success = sys_hashmap_put(map, "key1", false, 100, &replaced);
  test_assert(success == true);
  test_assert(replaced == 0); // No previous value

  uintptr_t value = sys_hashmap_get_value(map, "key1");
  test_assert(value == 100);

  // Test multiple keys
  test_assert(sys_hashmap_put(map, "key2", false, 200, NULL));
  test_assert(sys_hashmap_put(map, "key3", false, 300, NULL));
  ...
  test_assert(sys_hashmap_get_value(map, "nonexistent") == 0);

  // Test reverse lookup
  const char *key = sys_hashmap_get_key(map, 200);
  test_assert(key != NULL);
  test_assert(strcmp(key, "key2") == 0);

  // Test non-existent value
  test_assert(sys_hashmap_get_key(map, 999) == NULL);
...
int test_hashmap_long_keys(void) {
  ...
  test_assert(sys_hashmap_put(map, short_key, true, 100, NULL));
  test_assert(sys_hashmap_get_value(map, short_key) == 100);
  ...
  test_assert(sys_hashmap_put(map, long_key, true, 200, NULL));
  test_assert(sys_hashmap_get_value(map, long_key) == 200);

  // Very long key
  char very_long_key[200];
  memset(very_long_key, 'x', 199);
  very_long_key[199] = '\0';
  test_assert(sys_hashmap_put(map, very_long_key, true, 300, NULL));
  test_assert(sys_hashmap_get_value(map, very_long_key) == 300);
...
int test_hashmap_key_copying(void) {
  ...
  test_assert(sys_hashmap_put(map, stack_key, false, 100, NULL));
  test_assert(sys_hashmap_get_value(map, stack_key) == 100);

  // Test copy=true (copy key)
  char original_key[] = "original_key";
  test_assert(sys_hashmap_put(map, original_key, true, 200, NULL));
  test_assert(sys_hashmap_get_value(map, original_key) == 200);

  // Modify original - copied key should still work
  original_key[0] = 'X'; // Change first character
  test_assert(sys_hashmap_get_value(map, "original_key") == 200); // Still works
  test_assert(sys_hashmap_get_value(map, original_key) ==
              0); // Modified key not found

  // Test long key copying
  char long_original[] =
      "this_is_a_long_key_that_gets_copied_and_requires_malloc_storage";
  test_assert(sys_hashmap_put(map, long_original, true, 300, NULL));
  test_assert(sys_hashmap_get_value(map, long_original) == 300);

  // Modify original long key
  long_original[0] = 'X';
  test_assert(
      sys_hashmap_get_value(
          map,
          "this_is_a_long_key_that_gets_copied_and_requires_malloc_storage") ==
      300);
  test_assert(sys_hashmap_get_value(map, long_original) == 0);
...
int test_hashmap_collisions(void) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    test_assert(
        sys_hashmap_put(map, keys[i], true, (uintptr_t)(100 + i), NULL));
  }

  // Verify all keys can be retrieved
  for (int i = 0; i < 10; i++) {
    uintptr_t value = sys_hashmap_get_value(map, keys[i]);
...
int test_hashmap_overwriting(void) {
  ...
  // Initial insert
  uintptr_t replaced = 0;
  test_assert(sys_hashmap_put(map, "test_key", true, 100, &replaced));
  test_assert(replaced == 0); // No previous value
  test_assert(sys_hashmap_get_value(map, "test_key") == 100);

  // Overwrite with same key
  test_assert(sys_hashmap_put(map, "test_key", true, 200, &replaced));
  test_assert(replaced == 100); // Previous value returned
  test_assert(sys_hashmap_get_value(map, "test_key") == 200);

  // Overwrite again
  test_assert(sys_hashmap_put(map, "test_key", true, 300, &replaced));
  test_assert(replaced == 200);
  test_assert(sys_hashmap_get_value(map, "test_key") == 300);

  // Test overwriting without capturing replaced value
  test_assert(sys_hashmap_put(map, "test_key", true, 400, NULL));
  test_assert(sys_hashmap_get_value(map, "test_key") == 400);
  ...
  test_assert(sys_hashmap_put(map, long_key, true, 500, NULL));
  test_assert(sys_hashmap_get_value(map, long_key) == 500);

  sys_puts("Testing long key overwrite...\n");
  test_assert(sys_hashmap_put(map, long_key, true, 600, &replaced));
  sys_puts("Long key overwrite completed.\n");
  test_assert(replaced == 500);
  test_assert(sys_hashmap_get_value(map, long_key) == 600);
