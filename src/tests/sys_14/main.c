...
static void consume_events_loop(dual_core_test_data_t *data,
  ...
  while (!atomic_load(&data->should_stop)) {
    ...
    if (event != NULL) {
      ...
      printf("%s consumed event %d (from Core %d), queue_size=%zu\n", core_name,
  ...
  while ((event = sys_event_queue_try_pop(&data->core0_to_core1_queue)) !=
    ...
    atomic_fetch_add(consumed_counter, 1);
    printf("%s drained event %d (from Core %d), queue_size=%zu\n", core_name,
           event_id, producer_core,
           sys_event_queue_size(&data->core0_to_core1_queue));
...
static void core1_consumer_task(void *arg) {
  ...
  if (!sys_timer_start(&data->core1_timer)) {
    ...
    printf("✗ Failed to start timer on core 1\n");
#else
    printf("✗ Failed to start timer on consumer thread\n");
#endif
    atomic_store(&data->should_stop, true);
    return;
  }
#ifdef SYSTEM_NAME_PICO
  printf("✓ Core 1 timer started\n");
#else
  printf("✓ Consumer thread timer started\n");
#endif
  ...
  printf("Consumer thread finished (consumed %d events)\n",
         atomic_load(&data->core1_consumed));
#endif
}
...
int test_sys_14(void) {
...
#else
  printf("Running on macOS/Linux platform with threading\n");
  // On macOS/Linux platforms, we use regular threading instead of core-specific
  // threads
  printf("This test validates multi-threaded event queue behavior\n");
#endif
  ...
  printf("✓ Timers initialized (25ms intervals)\n");
  ...
  if (!sys_thread_create(core1_consumer_task, &test_data)) {
    printf("✗ Failed to start consumer thread\n");
    sys_timer_finalize(&test_data.core0_timer);
    sys_timer_finalize(&test_data.core1_timer);
    sys_event_queue_finalize(&test_data.core0_to_core1_queue);
    return 1;
...
#ifdef SYSTEM_NAME_PICO
    printf("✗ Failed to start timer on core 0\n");
#else
    printf("✗ Failed to start timer on main thread\n");
#endif
    atomic_store(&test_data.should_stop, true);
    return 1;
  }
#ifdef SYSTEM_NAME_PICO
  printf("✓ Core 0 timer started\n");
#else
  printf("✓ Main thread timer started\n");
#endif
  ...
  sys_timer_finalize(&test_data.core0_timer);
  sys_timer_finalize(&test_data.core1_timer);
  printf("✓ Timers stopped and finalized\n");
  ...
  printf("Core 0 starting to drain remaining events...\n");
  sys_event_t event;
  while ((event = sys_event_queue_try_pop(&test_data.core0_to_core1_queue)) !=
    ...
    printf("Core 0 drained event %d (from Core %d), queue_size=%zu\n", event_id,
           producer_core,
  ...
  if (total_timer_events != 100) {
    printf("✗ Expected 100 timer events, got %d\n", total_timer_events);
    return 1;
  }

  if (total_consumed != 100) {
    printf("✗ Expected 100 total consumed events, got %d\n", total_consumed);
    return 1;
  }
  ...
  if (core0_consumed == 0) {
    printf("✗ Main thread didn't consume any events\n");
    return 1;
  }
  if (core1_consumed == 0) {
    printf("✗ Consumer thread didn't consume any events\n");
    return 1;
  }
  printf("✓ Both threads successfully consumed events from shared queue\n");
#endif
