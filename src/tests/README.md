...
| NXFoundation_18 | String Method Testing | Tests NXString comprehensive functionality. String creation methods (`stringWithCString:`, `stringWithFormat:`, `stringWithString:`, `stringWithCapacity:`, `initWith...` variants), string comparison methods (`isEqual:`, `compare:`), character counting (`countOccurrencesOfByte:`), prefix/suffix testing (`hasPrefix:`, `hasSuffix:`), case conversion methods (`toUppercase`, `toLowercase`), string appending methods (`append:`, `appendCString:`, `appendStringWithFormat:` with format specifiers, numbers, mixed types, empty strings, capacity management), whitespace trimming (`trimWhitespace` with leading/trailing spaces, tabs, newlines, mixed whitespace, empty strings, whitespace-only strings, internal whitespace preservation), edge cases (self-appending, memory reallocation, immutable to mutable conversion), and string lifecycle management. |
